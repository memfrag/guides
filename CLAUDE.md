...
- `assets/style.css` is the shared stylesheet for the index and any guide that opts into it. It is dark-only, and its palette and type stack are deliberately the same as `docs/swiftui-alignment.html` (`--bg #191b20`, `--guide` orange for rules and metadata, `--view` cyan for content and links, system sans + mono) so the index and the guides read as one publication. It also defines the shared classes (`.guide-list`, `.tag`, `.back-link`, `.rail`, `.dashed`, etc.). No web fonts are loaded — the type is system sans and system mono.
